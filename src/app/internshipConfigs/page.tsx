...
export default function InternshipConfigsPage() {
  ...
  const {
    mutateAsync: updateInternshipConfig,
  ...
  const handleUpdatePeriodTotalHours = async (
    periodConfig: InternshipConfigType
  ) => {
  ...
  const firstPeriod = internshipConfigs.find(
    (config) => config.period === "11"
  ) as InternshipConfigType;
  const secondPeriod = internshipConfigs.find(
    (config) => config.period === "12"
  ) as InternshipConfigType;
    ...
    <div className="w-screen h-screen">
      ...
      <div className="px-4 py-3 flex flex-col gap-4">
        <Card>
          <CardHeader>
            <CardDescription>
              Configure aqui os parâmetros do estágio para o curso que direges
            </CardDescription>
          </CardHeader>
